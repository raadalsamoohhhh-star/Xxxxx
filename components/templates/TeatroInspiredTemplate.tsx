...
type Props = {
  ...
  invitationText:string; heroImage?:string; mapUrl?:string; rsvpEnabled?:boolean; monogram?:string; gallery?:string[]; sections?: { story:boolean; schedule:boolean; gallery:boolean; venue:boolean; rsvp:boolean; wishes:boolean }; sectionOrder?: string[];
};

export default function TeatroInspiredTemplate({groom,bride,date,venue,address,invitationText,heroImage,mapUrl,rsvpEnabled=true,monogram="M & L",gallery=[],sections,sectionOrder=["story","schedule","gallery","venue","rsvp","wishes"]}:Props){
  ...
  return <div className="teatro-template">
    ...
    <section className="teatro-footer-actions">{mapUrl&&<a href={mapUrl} target="_blank" rel="noreferrer">📍 الخريطة</a>}
