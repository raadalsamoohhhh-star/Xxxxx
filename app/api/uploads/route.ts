...
export async function POST(req:Request){
  const ip=req.headers.get('x-forwarded-for')?.split(',')[0]?.trim()||'unknown';if(!rateLimit(`upload:${ip}`,30,60_000))return NextResponse.json({error:'محاولات رفع كثيرة. حاول لاحقًا.'},{status:429});const user=await getCurrentUser();if(!user)return NextResponse.json({error:'غير مصرح'},{status:401});
