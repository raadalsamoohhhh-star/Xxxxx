  ...
  return (
    <main className="min-h-screen bg-[#f4eee6] p-3 sm:p-6">
      <div className="max-w-3xl mx-auto">
        ...
        {(data.videoUrl || data.heroImage) && <div className="mt-5 rounded-[32px] overflow-hidden bg-black shadow-xl">{data.videoUrl ? <video src={data.videoUrl} controls playsInline className="w-full max-h-[720px] object-cover" poster={data.heroImage || undefined}/> : <img src={data.heroImage} className="w-full max-h-[720px] object-cover"/>}</div>}
        {data.musicUrl && <div className="mt-5 rounded-3xl bg-white p-5 shadow-xl"><p className="text-sm font-bold mb-3">موسيقى الدعوة</p><audio controls src={data.musicUrl} className="w-full"/></div>}
        {!!data.gallery?.length && <section className="mt-5 rounded-[32px] bg-white p-6 shadow-xl"><h2 className="serif text-3xl text-center">ذكرياتنا</h2><div className="grid grid-cols-2 sm:grid-cols-3 gap-3 mt-5">{data.gallery.map((src,i)=><img key={src+i} src={src} className="w-full aspect-square object-cover rounded-2xl" loading="lazy"/>)}</div></section>}
